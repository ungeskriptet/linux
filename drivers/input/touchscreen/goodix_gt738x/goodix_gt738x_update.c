...
/*
		for(i =0;i<4;i++)
			{
		sprintf(tempchar,"%02x",fw_ver.vid[i]);
	...
	//	ts_info("hardwareinfo %s",hardinfo);
		...
		ts_info("hardwareinfo %s",hardinfo);
		register_hardware_info("TP_gt738x", hardinfo);
	*/
...
static int goodix_wait_sta(struct goodix_ts_device *ts_dev, u16 sta_addr, u8 sta, u16 timeout)
{
	u16 i, cnt;
	u8 data;
			   ...
			   struct firmware_data *fw_data)
{
	...
	for (i = 0; i < fw_data->fw_info.subsys_num; i++) {
		if (SS51_ISP_CODE == fw_data->fw_info.subsys[i].type) {
...
static int goodix_update_prepare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	for (i = 0; i < retry_cnt; i++) {
		...
		ret = goodix_reg_write(ts_dev, HW_REG_SPI_ACCESS, &spi_dis, len);	
		if (ret >= 0) {	
			ret = goodix_reg_read(ts_dev, HW_REG_SPI_STATE, &spi_dis, len);
			if (ret >= 0 && (spi_dis & SPI_STATUS_FLAG) == 0)
				break;
		}

		spi_dis = 2; /* disable G1 */
		ret = goodix_reg_write(ts_dev, HW_REG_G1_ACCESS1, &spi_dis, len);
		if (ret < 0 && i == retry_cnt-1)
			ts_err("write 0x4255 fail\n");
		ret = goodix_reg_write(ts_dev, HW_REG_G1_ACCESS2, &spi_dis, len);
		if (ret < 0 && i == retry_cnt-1)
...
static int goodix_set_checksum(struct goodix_ts_device *ts_dev,u8 *code, u32 len)
{
	u32 chksum = 0;
	u32 tmp;
	u32 i;
...
static int goodix_send_fw_packet(struct goodix_ts_device *ts_dev,struct fw_subsys_info *fw_x )
{
	...
	while (remain_len > 0) {
		...
		for (j = 0; j < retry_cnt; j++) {
			/* set packet size */
			ret = goodix_write_u32(ts_dev,HW_REG_PACKET_SIZE, current_len);
			if (ret < 0) {
				ts_err("%s:set packet size failed\n",__func__);
				continue;
			}
			/* set flash addr */
			ret = goodix_write_u32(ts_dev, HW_REG_PACKET_ADDR, write_addr);
			if (ret < 0) {
			...
			ret = goodix_reg_write_confirm(ts_dev, HW_REG_RAM_ADDR, fw_addr,current_len);
			if (ret < 0) {
			...
			ret = goodix_wait_sta(ts_dev, HW_REG_DOWNL_STATE, START_WRITE_FLASH, time_delay);
			if (ret < 0) {
			...
			ret = goodix_wait_sta(ts_dev, HW_REG_DOWNL_STATE, WRITE_OVER2_FLAG, time_delay);
			if (ret < 0) {
				ts_err("%s:write over2 failed\n",__func__);
				continue;
			}
			time_delay = 600; /* delay 600ms to write flash */
			ret = goodix_wait_sta(ts_dev, HW_REG_DOWNL_RESULT, WRITE_TO_FLASH_FLAG, time_delay);
			if (ret < 0) {
