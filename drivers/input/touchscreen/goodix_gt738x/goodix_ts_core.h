...
#define GOODIX_CFG_LEN			444
#define GOODIX_RETRY_NUM_20		20
#define GOODIX_RETRY_NUM_10		10
...
struct goodix_ts_hw_ops {
	...
	int (*write)(struct goodix_ts_device *dev, unsigned int addr,
			unsigned char *data, unsigned int len);
	int (*read_nodelay)(struct goodix_ts_device *dev, unsigned int addr,
			 unsigned char *data, unsigned int len);
	int (*write_nodelay)(struct goodix_ts_device *dev, unsigned int addr,
			unsigned char *data, unsigned int len);
	...
	int (*init_esd)(struct goodix_ts_device *dev);
...
//int goodix_close_hidi2c_mode(struct goodix_ts_device *dev);/*debug for haiquan by yzt*/
#ifndef CONFIG_TOUCHSCREEN_GOODIX_GT738X_UPDATE
