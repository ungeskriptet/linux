	...
	unsigned char *data, unsigned int len)
{
	...
	if (likely(len < sizeof(get_buf))) {
		...
		msgs[1].buf = kzalloc(I2C_MAX_TRANSFER_SIZE < len
				   ? I2C_MAX_TRANSFER_SIZE : len, GFP_KERNEL|GFP_DMA);
		if (msgs[1].buf == NULL)
			return -ENOMEM;
	}
...
static int goodix_remap_trace_id(
		u8 *coor_buf, u32 coor_buf_len, int touch_num)
{
	...
	if (!coor_buf || coor_buf_len > (3 + BYTES_PER_COORD * max_touch_num+4)) {
		ts_info("touch data buff error, !coor_buf:%d, len:%d", !coor_buf, coor_buf_len);
		return 0;
	}
	...
	struct goodix_pen_data *pen_data, unsigned char *buf, int touch_num)
{
	...
	/* process pen button */
	cur_key_map = (coor_data[BYTES_PEN_COORD] >> 4) & 0x03;
PEN_KEY_PROCESS:
		...
		u8 *pre_buf, u32 pre_buf_len)
{
	...
	static u8 buffer[IRQ_HEAD_LEN + BYTES_PER_COORD * (GOODIX_MAX_TOUCH-1) + 
				BYTES_PEN_COORD+ 2];
	...
	if (have_pen)
		goodix_remap_trace_id(&buffer[IRQ_HEAD_LEN], 
				BYTES_PER_COORD * GOODIX_MAX_TOUCH, touch_num-1);
	else
		goodix_remap_trace_id(&buffer[IRQ_HEAD_LEN], 
