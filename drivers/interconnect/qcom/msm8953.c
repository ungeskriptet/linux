...
static struct qcom_icc_node *msm8953_snoc_mm_nodes[] = {
	[MAS_JPEG] = &mas_jpeg,
	[MAS_MDP] = &mas_mdp,
	[MAS_VENUS] = &mas_venus,
	[MAS_VFE0] = &mas_vfe0,
...
int msm8953_qnoc_probe(struct platform_device *pdev)
{
	int ret = qnoc_probe(pdev);

	if (ret < 0)
		return ret;

	if (of_device_is_compatible(pdev->dev.of_node, "qcom,msm8953-snoc"))
